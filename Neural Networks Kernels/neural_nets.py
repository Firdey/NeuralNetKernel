...
class Network:
    ...
    and holds methods for training, testing, prediction.
    ...
    def batch_update(self, batch, n, step, lmbd):
        ...
        self.w_change = [(step*lmbd/n*w + step/len(batch)*nw + self.moment * wch)
                        for w, nw, wch in zip(self.weights, self.w_grad, self.w_change)]
        self.b_change = [step/len(batch)*nb + self.moment * bch
        ...
        self.weights = [w - wch for w,wch in zip(self.weights, self.w_change)]
        self.biases = [b - bch for b,bch in zip(self.biases, self.b_change)]
